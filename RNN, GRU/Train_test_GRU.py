...
class RNN(nn.Module):
  def __init__(self, intput_size, hidden_size, num_layers, num_classes):
    ...
    self.rnn = nn.GRU(input_size, hidden_size, num_layers, batch_first=True, dropout=0.4)
...
for epoch in range(num_epochs):
  ...
  print("===Epoch [{}/{}] Step[{}/{}]===\ntrain loss: {:.4f}, train accuracy: {:.4f}".format(epoch+1,num_epochs, i+1, total_step, loss.item(), train_acc_value))
  ...
  if best_model_loss > val_loss_value :
    ...
    best_epoch = epoch
    print("your model is saved as GRU_epoch_{:02d}.pth\n\n".format(best_epoch+1))
    torch.save(model.state_dict(), "GRU_epoch_{:02d}.pth".format(best_epoch+1))
...
# calculate accuracy of test dataset with each model
import os
...
for i in range(len(file_list_pth)):
  test_model = RNN(input_size, hidden_size, num_layers, num_classes).to(device)
  test_model.load_state_dict(torch.load('/content/'+file_list_pth[i], map_location=torch.device('cpu')))
  test_model.eval()
  correct = 0
  ...
  with torch.no_grad():
    ...
    print('Test Accuracy of RNN model:{} is: {}%'.format(file_list_pth[i], 100 * correct / len(test_data)))

## plot accuracy curve of test set

loss_plot = plt.figure(figsize=(15,9))
loss_plot = plt.plot(test_acc_list, label = 'test')


loss_plot = plt.title('test accuray curve')
loss_plot = plt.xlabel('accuracy')
loss_plot = plt.ylabel('Epoch')
